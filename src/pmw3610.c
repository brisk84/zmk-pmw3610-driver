...
static bool is_layer_ignored(uint8_t layer) {
    ...
    for (size_t i = 0; i < config->automouse_ignored_layers_len; i++) {
        if (layer == config->automouse_ignored_layers[i]) {
            return true;
        }
    }
    return false;
...
#if AUTOMOUSE_LAYER > 0
    // Store device pointer for automouse layer checking
    pmw3610_dev = dev;
#endif
...
#define PMW3610_DEFINE(n)                                                                          \
    ...
    static int32_t automouse_ignored_layers##n[] = DT_PROP(DT_DRV_INST(n), automouse_ignored_layers); \
    static const struct pixart_config config##n = {                                                \
        ...
        .automouse_ignored_layers = automouse_ignored_layers##n,                                   \
        .automouse_ignored_layers_len = DT_PROP_LEN(DT_DRV_INST(n), automouse_ignored_layers),     \
    };                                                                                             \
                                                                                                   \
